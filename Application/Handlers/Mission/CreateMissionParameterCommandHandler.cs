...
internal class CreateMissionParameterCommandHandler : ICommandHandler<CreateMissionParameterCommand, MissionParameterDto>
{
    ...
    {
        var totalWeight = new MeasureandQuantity(request.TotalStructureWeight_kg, SiPrefixes.Kilo.Name + SiUnits.Mass.Name);
        var payloadWeight = new MeasureandQuantity(request.PayloadWeight_kg, SiPrefixes.Kilo.Name + SiUnits.Mass.Name);
        var flightTime = TimeSpan.FromMinutes(request.FlightTimeInMinutes);

        var missionPrameter = new MissionParameterEstimates(Guid.NewGuid(), totalWeight, payloadWeight, flightTime);
